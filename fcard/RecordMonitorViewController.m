...
@interface RecordMonitorViewController ()

@end

@implementation RecordMonitorViewController
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        if (str != nil) {
            [database executeUpdate:@"INSERT INTO card(card_num, card_name, card_tel, card_info) VALUES(?,?,?,?)",
             str, @"未填写", @"", @"备注"];
        }
    }
    return;
...
- (void) search:(int)page
{
    ...
    [_indictor startAnimating];
    [MyTool intToChars:(recordCount - pageCount+1) chars:index];
...
- (void)waiting
{
    dispatch_queue_t wait = dispatch_queue_create("waitReboot", nil);
    dispatch_async(wait, ^{
        while (waitTime > 0 && isrunning) {
            waitTime--;
            dispatch_async(dispatch_get_main_queue(), ^{
                [_mTitle setText: [NSString stringWithFormat:@"等待刷新时间 %d s",waitTime]];
            });
            if (waitTime == 1)
            {
                NSString *str = [MyKeyChainHelper getRateWithService:KEY_RATE];
